...
(define (mupllist->racketlist mxs)
  (cond [(aunit? mxs) null]
        [#t (cons (apair-e1 mxs) (mupllist->racketlist (apair-e2 mxs)))]))
...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(fun? e) (closure env e)]          
        [(call? e) (let ([fexp (eval-under-env (call-funexp e) env)]
                         [actual-arg (eval-under-env (call-actual e) env)])
                     (if (closure? fexp)
                         (let* ([fenv (closure-env fexp)]
                                [f (closure-fun fexp)]
                                [fname (fun-nameopt f)]
                                [farg (fun-formal f)]
                                [fbody (fun-body f)])
                           (if fname
                               (eval-under-env fbody                                             
                                               (cons (cons farg actual-arg)
                                               (cons (cons fname fexp) fenv)))                                                                                                          
                               (eval-under-env fbody
                                               (cons (cons farg actual-arg)
                                                     fenv))))
...
(define mupl-map
  (fun "map" "mf"
       (fun "_f" "mlst"
            (ifaunit (var "mlst")
                     (aunit)
                     (apair (call (var "mf") (fst (var "mlst")))
                            (call (var "_f") (snd (var "mlst"))))))))                                      
